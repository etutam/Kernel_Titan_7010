...
unsigned long bInputAudioSampleFreq	= INPUT_AUDIO_SAMPLE_FREQ ;
int bInputAudioChannel = INPUT_AUDIO_CHANNEL ;
int bInputAudioFormat = 16;
...
HDMITX_SetOutput()
{
	extern int cat6612_spdif_enable,cat6612_hdcp_enable;	// WMT Sam Shen
    ...
    if( bHDMIMode )
    {
        ConfigAVIInfoFrame() ;
        DelayMS(100) ;            
     	EnableHDCP(cat6612_hdcp_enable) ;	// WMT Sam Shen
        
		if( bAudioEnable )
		{
		    SetNonPCMAudio(0) ; // For LPCM audio. If AC3 or other compressed audio, please set the parameter as 1
            EnableAudioOutput(VideoPixelClock*(pixelrep+1),bInputAudioSampleFreq, bInputAudioChannel, cat6612_spdif_enable);
            ConfigAudioInfoFrm() ;
		}
    }
    SetAVMute(FALSE) ;
...
void HDMITX_SetAudio(void)
{
#if 0
//	printk("[CAT6612] HDMITX_SetAudio freq %d\n",bInputAudioSampleFreq);
